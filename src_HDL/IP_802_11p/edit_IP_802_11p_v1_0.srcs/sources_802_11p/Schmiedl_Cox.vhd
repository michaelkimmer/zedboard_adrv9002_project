...
begin
    ...
    if rising_edge(CLOCK) then
        ...
        if DATA_STROBE = '1' then
            ...
            P_I <= P_I - resize(P_PIECE_I_MEM(L-L_OFF-1), P_I'LENGTH) + resize(P_PIECE_I(31 downto 4), P_I'LENGTH);
            P_Q <= P_Q - resize(P_PIECE_Q_MEM(L-L_OFF-1), P_Q'LENGTH) + resize(P_PIECE_Q(31 downto 4), P_Q'LENGTH);
            R   <= R   - resize(R_PIECE_MEM(L-L_OFF-1), R'LENGTH) + resize(R_PIECE(31 downto 4), R'LENGTH);
