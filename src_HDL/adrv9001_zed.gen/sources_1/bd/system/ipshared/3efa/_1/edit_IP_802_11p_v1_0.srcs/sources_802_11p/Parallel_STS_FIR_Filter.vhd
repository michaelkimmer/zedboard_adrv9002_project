...
entity Parallel_STS_FIR_Filter is
    ...
    Port ( 
           ...
           POWDATA_OUT_XCORR   : out std_logic_vector (60 downto 0) := (others => '0');
           -- not used !!
           --    IDATA_OUT_XCORR        : out std_logic_vector (35 downto 0) := (others => '0'); 
           --    QDATA_OUT_XCORR        : out std_logic_vector (35 downto 0) := (others => '0');
           

           STS_AUTOCORR_I : out std_logic_vector(2*INPUT_WIDTH-1+4 downto 0) := (others => '0'); -- 36b
           STS_AUTOCORR_Q : out std_logic_vector(2*INPUT_WIDTH-1+4 downto 0) := (others => '0') -- 36b
    );
...
architecture Behavioral of Parallel_STS_FIR_Filter is
    ...
    constant MAC_MULTIPLY_WIDTH : integer range 0 to 63 := 2*INPUT_WIDTH; --32b (31b used!)
    constant MAC_ADDING_WIDTH   : integer range 0 to 63 := MAC_MULTIPLY_WIDTH; -- 32b
    constant ACCUMULATOR_WIDTH  : integer range 0 to 63 := MAC_ADDING_WIDTH + 4;  -- 36b (added 16 samples --> +4b)
  
    -- Input buffer regs (Timing -- make 1 before each MAC)
    type input_registers is array(0 to FILTER_TAPS-1) of signed(INPUT_WIDTH-1 downto 0); -- only 14b !
    ...
    -- Accumulator output delay sync regs (not used !!)
    ...
    -- Power output
    signal POWER_II  :  signed(71 downto 0) := (others=>'0');
    signal POWER_QQ  :  signed(71 downto 0) := (others=>'0');
    signal POWER     :  unsigned(60 downto 0) := (others=>'0');


    -- STS autocorrelation computation (delayed by 16)
    signal INPUT_REG_I_0    : signed(INPUT_WIDTH-1 downto 0) := (others=> '0');
    signal INPUT_REG_Q_0    : signed(INPUT_WIDTH-1 downto 0) := (others=> '0');
    signal INPUT_REG_I_16 : signed(INPUT_WIDTH-1 downto 0) := (others=> '0');
    signal INPUT_REG_Q_16 : signed(INPUT_WIDTH-1 downto 0) := (others=> '0');

    signal AUTOCORR_MULT_REG_II : signed(2*INPUT_WIDTH-1 downto 0)  := (others=> '0'); --32b (msb(==30)) 
    ...
    signal AUTOCORR_PIECE_I : signed(2*INPUT_WIDTH-1 downto 0)  := (others=> '0'); -- 32b (all used)
    signal AUTOCORR_PIECE_Q : signed(2*INPUT_WIDTH-1 downto 0)  := (others=> '0');

    -- autocorr memory 127 samples long!, width just 29b !
    type AUTOCORR_MEM_t is array (0 to 127) of signed(2*INPUT_WIDTH-1-3 downto 0);
    signal AUTOCORR_I_MEM : AUTOCORR_MEM_t  := (others=>(others=> '0'));
    signal AUTOCORR_Q_MEM : AUTOCORR_MEM_t  := (others=>(others=> '0'));

    signal AUTOCORR_I : signed(2*INPUT_WIDTH-1+4 downto 0) := (others=>'0'); -- 36b
    signal AUTOCORR_Q : signed(2*INPUT_WIDTH-1+4 downto 0) := (others=>'0');


    type autocorr_delay_registers is array(0 to 1) of signed(2*INPUT_WIDTH-1+4 downto 0); -- 36b
    ...
    if rising_edge(CLOCK) then
        ...
        if DATA_IN_STROBE = '1' then
            ...
            for i in 0 to FILTER_TAPS-1 loop
                ...
                INPUT_REG_I(i) <= signed(IDATA_IN(15 downto 0));  
                INPUT_REG_Q(i) <= signed(QDATA_IN(15 downto 0)); 

                -- Multiply input and resize back [16b+16b=32b (31b used !)]  -- Note: multiplplying is resized automatically [delay=+1]
                MULT_REG_II(i) <= (INPUT_REG_I(i) * COEFFS_I(i)); 
                MULT_REG_IQ(i) <= (INPUT_REG_I(i) * COEFFS_Q(i)); 
                MULT_REG_QI(i) <= (INPUT_REG_Q(i) * COEFFS_I(i)); 
                MULT_REG_QQ(i) <= (INPUT_REG_Q(i) * COEFFS_Q(i)); 

                -- Add complex multiplies [32b]  -- Note: Adding needs to be resized manually! [delay=+1]
                ...
                -- Add complex Multiplies to accumulators [32b+4b=36b] [delay=+1]
                ...
                -- Compute output power (actual sq. of XCORR amplitude) (+delay accumulator for output) [delay=+2] [output 72b, used 70b !] -- 18x25 DSP block --> fit into it ?? !!!
                POWER_II <= (ACCUMULATOR_I(0) * ACCUMULATOR_I(0));
                POWER_QQ <= (ACCUMULATOR_Q(0) * ACCUMULATOR_Q(0));
                -- ACCUMULATOR_I_DELAY(1) <= ACCUMULATOR_I(0);
                -- ACCUMULATOR_Q_DELAY(1) <= ACCUMULATOR_Q(0);

                POWER <= unsigned(POWER_II(71 downto 11)) + unsigned(POWER_QQ(71 downto 11)); -- [61b, used 60b !]
...
sts_autocorr_computation : process(CLOCK)
begin
    ...
    if rising_edge(CLOCK) then
        ...
        if DATA_IN_STROBE = '1' then
            ...
            INPUT_REG_I_0 <= signed(IDATA_IN);
            INPUT_REG_Q_0 <= signed(QDATA_IN);
            ...
            AUTOCORR_MULT_REG_II <= INPUT_REG_I_0 * INPUT_REG_I_16;
            AUTOCORR_MULT_REG_IQ <= INPUT_REG_I_0 * INPUT_REG_Q_16;
            AUTOCORR_MULT_REG_QI <= INPUT_REG_Q_0 * INPUT_REG_I_16;
            AUTOCORR_MULT_REG_QQ <= INPUT_REG_Q_0 * INPUT_REG_Q_16;


            -- Add complex multiplies (32b, all used now)   
            AUTOCORR_PIECE_I <= AUTOCORR_MULT_REG_II - (-AUTOCORR_MULT_REG_QQ);
            AUTOCORR_PIECE_Q <= (-AUTOCORR_MULT_REG_IQ) + AUTOCORR_MULT_REG_QI;

            -- Accumulate last 128 autocorr pieces [+7b (cut -3b) --> 32+7-3b=36b]

                -- Save AUTOCORR pieces into memory (32-3b=29b) 
            AUTOCORR_I_MEM(0) <= AUTOCORR_PIECE_I(31 downto 3);
            AUTOCORR_I_MEM(1 to AUTOCORR_I_MEM'LENGTH-1) <= AUTOCORR_I_MEM(0 to AUTOCORR_I_MEM'LENGTH-2);
            AUTOCORR_Q_MEM(0) <= AUTOCORR_PIECE_Q(31 downto 3);
            AUTOCORR_Q_MEM(1 to AUTOCORR_Q_MEM'LENGTH-1) <= AUTOCORR_Q_MEM(0 to AUTOCORR_Q_MEM'LENGTH-2);

                -- Update accumulated autocorrelation (29+7b=36b)
            AUTOCORR_I <= AUTOCORR_I - resize(AUTOCORR_I_MEM(AUTOCORR_Q_MEM'LENGTH-1), AUTOCORR_I'LENGTH) + resize(AUTOCORR_PIECE_I(31 downto 3), AUTOCORR_I'LENGTH);
            AUTOCORR_Q <= AUTOCORR_Q - resize(AUTOCORR_Q_MEM(AUTOCORR_Q_MEM'LENGTH-1), AUTOCORR_Q'LENGTH) + resize(AUTOCORR_PIECE_Q(31 downto 3), AUTOCORR_Q'LENGTH);
    
            -- ADD & Delays -- synchronize with POWER computation (still 36b)
            AUTOCORR_DELAY_REG_I(1) <= AUTOCORR_I;
            AUTOCORR_DELAY_REG_Q(1) <= AUTOCORR_Q;
...
begin
    ...
    if rising_edge(CLOCK) then
        ...
        if RESET = '1' then
            ...
            if DATA_IN_STROBE = '1' then
                -- output XXb data [delay=+1]
                -- IDATA_OUT_XCORR       <= std_logic_vector(ACCUMULATOR_I_DELAY(0));  
                -- QDATA_OUT_XCORR       <= std_logic_vector(ACCUMULATOR_Q_DELAY(0)); 
                POWDATA_OUT_XCORR     <= std_logic_vector(POWER);

                STS_AUTOCORR_I <= std_logic_vector(AUTOCORR_DELAY_REG_I(0)); 
                STS_AUTOCORR_Q <= std_logic_vector(AUTOCORR_DELAY_REG_Q(0));
